...
DEFAULT_EMBEDDING_PROMPT = "Instruct: The following query comes from a psychological scale\nQuery:"
...
class SimplifyRequest(BaseModel):
    ...
    embedding_prompt: Optional[str] = DEFAULT_EMBEDDING_PROMPT
...
def simplify(req: SimplifyRequest):
    ...
    api_key = (req.api_key or os.getenv("DASHSCOPE_API_KEY") or "").strip()
    if not api_key:
        raise HTTPException(
            status_code=400,
            detail="Missing embedding API key. Enter a key in the web interface or set DASHSCOPE_API_KEY on the server.",
        )

    client = openai.OpenAI(api_key=api_key, base_url=req.base_url)
    embedding_model = OpenAIBackend(client, req.model_name, batch_size=10)

    # 2) Generate embeddings
    sentences = req.sentences
    embedding_sentences = sentences
    if req.embedding_prompt:
        embedding_sentences = [f"{req.embedding_prompt}{sentence}" for sentence in sentences]
    embeddings = embedding_model.embed(embedding_sentences)
