...
export default function QuestionnaireSimplifierApp() {
  ...
  const [consoleLog, setConsoleLog] = useState<string[]>([
    "System ready. Please paste the questionnaire text, prefix, and click 'Start Pre-processing'",
  ]);

  // Model settings
  const [apiKey, setApiKey] = useState<string>("");
  const [baseUrl, setBaseUrl] = useState<string>("https://dashscope.aliyuncs.com/compatible-mode/v1");
  const [modelName, setModelName] = useState<string>("text-embedding-v4");
  ...
  const [rawPlotPoints, setRawPlotPoints] = useState<RawPlotPoint[]>([]);
  ...
  async function handleRun() {
    ...
    try {
      ...
      setRawPlotPoints(
        (data.points || []).map((p) => ({ qid: p.qid, topic: p.topic, x: p.x, y: p.y }))
      );
      ...
      appendLog(`Error, info: ${getErrorMessage(err)}`);
    } finally {
      setRunning(false);
    }
  }
  ...
  async function handleVisualize() {
    try {
      ...
      const resp = await fetch("/api/plot2d", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
      const data: PlotResponse = await resp.json();
      ...
      setVizStatus(`Visualization failed, info: ${getErrorMessage(err)}`);
    }
  }
    ...
    <div className="min-h-screen bg-neutral-50 text-neutral-900">
      <header className="sticky top-0 z-10 backdrop-blur bg-white/70 border-b border-neutral-200">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="text-sm text-neutral-500">V1.1</div>
        </div>
      </header>
          ...
          <div className="rounded-2xl border border-neutral-200 bg-white shadow-sm">
            ...
            <div className="p-4 md:p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                {/* Question Prefix */}
                <div className="mt-3">
                  <label className="block text-sm font-medium mb-2">Question Prefix: </label>
                  <input
            ...
            <div className="p-4 md:p-6 grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="mt-2 inline-flex w-full items-center justify-center rounded-xl border border-blue-200 bg-blue-50 px-3 py-2 text-sm font-medium text-blue-700 transition hover:border-blue-300 hover:bg-blue-100"
                >
                  Click to apply Alibabacloud apikey.
                </a>
              </div>

              <div className="md:col-span-3 rounded-xl border border-neutral-200 bg-neutral-50">
                <button
                  type="button"
                  onClick={() => setAdvancedSettingsOpen((open) => !open)}
                  className="flex w-full items-center justify-between px-3 py-2 text-left text-sm font-medium text-neutral-800"
                  aria-expanded={advancedSettingsOpen}
                >
                  <span>Advanced Embedding API Settings</span>
                  <span className="text-neutral-500">{advancedSettingsOpen ? "Hide" : "Show"}</span>
                </button>

                {advancedSettingsOpen && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-t border-neutral-200 p-3">
                    <div>
                      ...
                      />
                      <p className="text-xs text-neutral-500 mt-1">
                        Default: Alibaba Cloud DashScope OpenAI-compatible endpoint.
                      </p>
                    </div>
              ...
              <div>
                ...
                <p className="text-xs text-neutral-500 mt-1">
                  Current online version uses Alibabacloud text-­embedding-v4 for demonstration. Results and figure may slightly vary from offline version.
                </p>
              </div>
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  ...
                  />
                  <p className="text-xs text-neutral-500 mt-1">
                    balance between preserving local versus global structure
                  </p>
                </div>
                <div>
                  ...
                  />
                  <p className="text-xs text-neutral-500 mt-1">
                    minimum number to form a cluster
                  </p>
                </div>
                <div>
                  ...
                  />
                  <p className="text-xs text-neutral-500 mt-1">
                    conservative of clustering considering core versus boundary points
                  </p>
                </div>
              </div>
            </div>
      ...
      <footer className="max-w-6xl mx-auto px-4 py-6 text-xs text-neutral-500">
        Psychological Scale Simplification Tool V1.1
      </footer>
    </div>
  );
}
...
function groupByTopic(rows: ResultRow[]) {
  const map = new Map<number, { topic: number; keywords: string; items: Pick<ResultRow, "text" | "prob">[] }>();
  for (const r of rows) {
